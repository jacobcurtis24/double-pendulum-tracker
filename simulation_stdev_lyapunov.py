...
globalYINIT_1=np.array([-pi*105/180,0,-pi*105/180,0])
...
for i in range(N):
	...
	#print "="*50,"\n",i, "/", N, disp1,disp2
	time, T1, T2=runSim(disp1,disp2)
	metaT1.append(T1)
	metaT2.append(T2)
	if i%10==0:
		ax1.plot(time,T1, "r-", label="$\Theta_1$")
		ax1.plot(time,T2, "b-", label="$\Theta_2$")
for lya_time in times:
	selected_positions1 = []
	selected_positions2 = []
	for i in metaT1:
		selected_positions1.append(i[lya_time])
		#print i[lya_time] * 180 / np.pi -90

	for i in metaT2:
		selected_positions2.append(i[lya_time])
		#print i[lya_time] * 180 / np.pi - 90

	lya_std1 = np.std(selected_positions1)
	lyapunov1 = (np.log(lya_std1) - np.log(variance) / 2) / (lya_time * 0.001)
	lyapunov1_total.append(lyapunov1)
	lya_std2 = np.std(selected_positions2)
	lyapunov2 = (np.log(lya_std2) - np.log(variance) / 2) / (lya_time * 0.001)
	lyapunov2_total.append(lyapunov2)
	print "At t =", lya_time * 0.001, " with STDEV: ", lya_std1, " and ", lya_std2
	print "Average angles are: ", np.average(selected_positions1) * 180 / np.pi - 90, " and ", np.average(selected_positions2) * 180 / np.pi - 90
	print "First Lyapunov exponent as determined with the same method used experimentally is: ", lyapunov1
	print "Second is: ", lyapunov2

print "Average first Lyapunov exponent is: ", np.average(lyapunov1_total)
print "Average second Lyapunov exponent is: ", np.average(lyapunov2_total)
