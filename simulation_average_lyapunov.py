...
globalYINIT_1=np.array([-pi*75/180,0,-pi*75/180,0])
...
average1 = np.average(selected_positions1)
average2 = np.average(selected_positions2)

for i in selected_positions1:
	lyapunov1.append((np.log(np.abs(i - average1)) - np.log(variance) / 2) / (lya_time / 1000))

for i in selected_positions2:
	lyapunov2.append((np.log(np.abs(i - average2)) - np.log(variance) / 2) / (lya_time / 1000))

print "At t =", lya_time * 0.001
print "Average first angle is: ", average1 * 180 / np.pi - 90
print "Average second angle is: ", average2 * 180 / np.pi - 90
print "First Lyapunov exponent as determined with averaging at each time is: ", np.average(lyapunov1)
print "Second is: ", np.average(lyapunov2)
